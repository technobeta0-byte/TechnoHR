...
import {
  ...
  FiRefreshCw, FiShield, FiCalendar, FiSettings, FiCreditCard,
...
export default function EmployeeHome() {
  const { employee, settings, setTodayRecord, getTodayRecordIfFresh } = useStore()

  // ── حالة الحضور - تبدأ من الـ cache فوراً (Offline-First) ──────────────
  ...
  const [showMore, setShowMore] = useState(false)  // قائمة "المزيد" للأسفل
  ...
  const handleRegisterBio = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setMsg({ type: 'success', text: 'تم تفعيل البصمة / Face ID بنجاح ✅' })
      } else {
        setMsg({ type: 'error', text: res.error || 'فشل حفظ البصمة' })
      }
    } catch (e) {
      setMsg({ type: 'error', text: e.message })
    } finally {
    ...
    <EmployeeLayout title={settings?.branch_name || 'TechnoHR'} subtitle={employee?.name}>
      <div className="px-4 pt-4 pb-6 space-y-4">
        ...
        <div className="card text-center space-y-1 border-gold-200 dark:border-gold-800/30 bg-gradient-to-br from-gold-50 to-white dark:from-gold-900/10 dark:to-dark-card">
          ...
          <p className="text-5xl font-black text-gray-900 dark:text-white tracking-widest tabular-nums">{timeStr}</p>
        ...
        <div className="card space-y-3">
          ...
          {hasIn && (
            <div className="space-y-3">
              ...
              {/* نسيان بصمة */}
              {isForgot && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-2xl p-4 space-y-1">
                  <div className="flex items-center gap-2 text-red-600 dark:text-red-400">
                    <FiAlertTriangle className="w-5 h-5 flex-shrink-0" />
                    <p className="font-bold text-sm">نسيان بصمة الانصراف</p>
                  </div>
                  <p className="text-xs text-red-500 dark:text-red-400">
                    حضور: <strong>{record.check_in_time}</strong> — انصراف تلقائي: <strong>{record.check_out_time}</strong>
                  </p>
                </div>
              )}
              ...
              {hasIn && hasOut && !isForgot && (
                <div className="bg-gray-50 dark:bg-dark-card2 rounded-2xl p-3 text-center">
                  <p className="text-xs text-gray-400 dark:text-dark-muted">إجمالي ساعات العمل</p>
        ...
        {hasIn && hasOut && !isForgot && (
          <div className="card text-center py-6 space-y-2 border-green-200 dark:border-green-800/30">
            ...
            <p className="font-bold text-lg text-gray-900 dark:text-white">تم تسجيل حضورك وانصرافك اليوم ✅</p>
          </div>
        )}
        ...
        <div className="card space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <InfoBox label="نوع الشيفت" value={
              ...
              : employee?.shift_type === 'flexible' ? 'مرن' : 'مختلط'
            } />
            <InfoBox label="رصيد الإجازات" value={`${employee?.annual_leave_days || 21} يوم`} />
            <InfoBox label="القسم" value={employee?.department || 'غير محدد'} />
          </div>
        </div>

        {/* ── قسم المزيد (طلب إجازة + إعدادات الأمان) ────────── */}
        <button
          onClick={() => setShowMore(v => !v)}
          className="w-full flex items-center justify-between px-2 py-1 text-sm text-gray-400 dark:text-dark-muted hover:text-gray-600 dark:hover:text-dark-text transition-colors"
        >
          <span className="font-semibold">إعدادات وخدمات إضافية</span>
          {showMore
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">سنوية، مرضية، طارئة...</p>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">البصمة / Face ID / NFC</p>
                </div>
              </div>
              <span className="text-xs text-gold-600 font-bold">فتح ←</span>
            </button>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="card w-full max-w-md bg-white dark:bg-dark-card space-y-4 animate-slide-up">
              <div className="flex justify-between items-center border-b border-gray-100 dark:border-dark-border pb-3">
                <h3 className="font-bold text-lg dark:text-white">إعدادات الأمان</h3>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {hasBioRegistered ? '🟢 مفعلة' : '⚪ غير مفعلة'}
                      </p>
                    </div>
                  </div>
                  <button onClick={handleRegisterBio} disabled={bioLoading} className="btn-gold px-4 py-2 text-xs w-auto">
                    {bioLoading ? '...' : (hasBioRegistered ? 'إعادة تفعيل' : 'تفعيل')}
                  </button>
                </div>
                {/* NFC */}
                <div className="p-3 bg-gray-50 dark:bg-dark-card2 rounded-2xl border border-gray-100 dark:border-dark-border flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {employee?.nfc_card_id ? `🟢 مربوط: ${employee.nfc_card_id}` : '⚪ غير مربوط (تواصل مع الأدمن)'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="card w-full max-w-md bg-white dark:bg-dark-card space-y-4 animate-slide-up">
              ...
              <form onSubmit={handleRequestLeave} className="space-y-3">
                ...
                <div>
                  ...
                  <textarea className="input-field h-20" placeholder="اكتب السبب..."
